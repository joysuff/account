...
**接口描述**：获取用户的的所有推送方式（例如 email、wechat 等）

**请求方法**：`GET`

**请求路径**：`/api/notify/methods`
  ...
  "data": [
    {
      ...
      "config": {},
      "enabled": 1,
      "created_at": "2025-10-27T02:47:15.000Z"
    },
    {
      "id": 3,
      "user_id": 2,
      ...
      "config": {
        "token": "XXXXXX",
        "server_url": "https://msg.768451.xyz"
      },
      "enabled": 0,
      "created_at": "2025-10-27T02:47:15.000Z"
    }
...
**接口描述**：获取当前登录用户已启用的推送配置或当前启用的推送方式

**请求方法**：`GET`

**请求路径**：`/api/notify/enabled`
...
**接口描述**：启用或禁用某条用户推送配置（修改 enabled 字段）

**请求方法**：`PUT`

**请求路径**：`/api/notify/enabled/:id`
...
| 参数名 | 类型   | 必填 | 说明                 |
| ------ | ------ | ---- | -------------------- |
| config | object | 是   | 推送配置内容（任意 JSON） |
...
**接口描述**：根据channelId获取推送方式的名称

**请求方法**：`GET`

**请求路径**：`/api/notify/method/name/:channelId`
